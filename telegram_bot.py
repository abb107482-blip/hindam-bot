...
async def on_message(u, c):
    ...
    if re.match(r"https?://[^\s]+", txt):
        ...
        if info:
            ...
            await u.message.reply_text("❌ Rabat ghayr saleh.")
        return
    if len(txt) > 1:
...
async def on_callback(u, c):
    ...
    if data.startswith("dv_") or data.startswith("da_"):
        ...
        if not url:
            await q.edit_message_text("❌ A3d irsal alrabat.")
            return
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            if err or not fp:
                await q.edit_message_text("❌ " + (err or "Fashal."))
                return
            if os.path.getsize(fp) > MAX_FILE_MB * 1024 * 1024:
                await q.edit_message_text("❌ File too large.")
                return
            try:
